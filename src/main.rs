...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    input
        ...
        .for_each(|line| *hm.entry(line).or_default() += 1); //For each line, all trimmed to remove extra spaces, and converted back to Strings to avoid ownership faffery. Then, in each iteration get a reference to an entry for the current kind, or the default and add one.
    ...
    let mut rows: Vec<(String, u64, String)> = hm
        .into_iter()
        .map(|(key, no)| (key, no, format!("{:.1}%", (no as f32) / total * 100.0)))
